...
hi chúng ta thực hiện phân tích gộp các thử nghiệm lâm sàng hoặc các loại nghiên cứu can thiệp khác, chúng ta thường ước tính mức độ ảnh hưởng thực sự của **một** phương pháp điều trị cụ thể. Chúng ta lựa chọn các nghiên cứu trong đó cùng một loại can thiệp được so sánh với các nhóm đối chứng tương tự, ví dụ giả dược. Tất cả những yêu tố khác là như nhau, điều này cho phép đánh giá xem một loại điều trị **cụ thể** có hiệu quả hay không. 

Tuy nhiên, trong nhiều lĩnh vực nghiên cứu, có không chỉ "giới hạn" một phương pháp điều trị--mà có nhiều hơn một. Ví dụ, chứng đau nửa đầu có thể được điều trị bằng nhiều loại thuốc khác nhau và các lựa chọn liệu pháp không dùng thuốc cũng tồn tại. Đặc biệt là trong các lĩnh vực nghiên cứu "không còn mới" việc chỉ ra rằng một số hình thức điều trị là có lợi thường ít ý nghĩa. Thay vào đó, chúng ta muốn tìm ra phương pháp điều trị hiệu quả **nhất** đối với một số hoàn cảnh cụ thể. 

Điều này dẫn đến những vấn đề mới. Để đánh giá hiệu quả so sánh của một số phương pháp điều trị trong một phân tích gộp thông thường, cần có đủ các so sánh trực tiếp giữa hai phương pháp điều trị. Tuy nhiên, thực tế không phải lúc nào cũng vậy. Trong nhiều lĩnh vực nghiên cứu, người ta thường thấy rằng chỉ có rất ít các thử nghiệm so sánh **một cách trực tiếp** tác dụng của hai phương pháp điều trị, thay cho các nhóm đối chứng "yếu hơn". Điều này thường có nghĩa là các phân tích gộp truyền thống không thể được sử dụng để thiết lập bằng chứng chắc chắn về hiệu quả **tương đối** của một số phương pháp điều trị.

Tuy nhiên, trong khi việc so sánh trực tiếp giữa hai hoặc nhiều phương pháp điều trị có thể không tồn tại, các bằng chứng **gián tiếp** thường có sẵn. Các phương pháp điều trị khác nhau có thể đã được đánh giá trong các thử nghiệm **riêng lẻ**, nhưng tất cả các thử nghiệm này có thể đã sử dụng **cùng một** nhóm chứng. Ví dụ, có thể hai loại thuốc chưa bao giờ được so sánh trực tiếp, nhưng tác dụng của cả hai loại thuốc so với giả dược dạng viên đã được nghiên cứu rộng rãi. 

\index{Mixed-Treatment Comparison Meta-Analysis}

**Phân tích gộp mạng tương quan (Network meta-analysis)** có thể được sử dụng để kết hợp các so sánh gián tiếp như vậy và do đó cho phép chúng ta so sánh tác động của một số biện pháp can thiệp một cách đồng thời [@dias2013evidence]. Phân tích gộp mạng tương quan còn được biết đến với tên **phân tích gộp so sánh nhiều phương phá điều trị (mixed-treatment comparison meta-analysis)** [@van2012automating]. Phương pháp này cho phép tích hợp nhiều so sánh điều trị trực tiếp và gián tiếp vào cùng một mô hình, có thể được cấu thành một “mạng lưới” so sánh.

\index{Consistency}

Phân tích gộp mạng tương quan là một chủ đề nghiên cứu “hot”. Trong thập kỷ qua, nó ngày càng được các nhà nghiên cứu ứng dụng trong lĩnh vực y sinh học và các ngành khoa học khác đón nhận. Tuy nhiên, phương pháp này cũng đi kèm với những thách thức và cạm bẫy, đặc biệt liên quan đến tính không đồng nhất và sự **không nhất quán của mạng** [@salanti2014evaluating]. 

Do đó, điều quan trọng trước tiên là phải thảo luận về các thành phần cốt lõi và các giả định của mô hình phân tích gộp mạng tương quan. Nền tảng của phân tích gộp mạng tương quan đôi khi có thể hơi trừu tượng. Do đó, chúng tôi sẽ đi qua các nội dung thiết yếu theo từng bước, để giúp bạn hiểu rõ hơn về phương pháp này.

<br></br>

## Phân tích gộp mạng tương quan là gì? {#what-is-net-ma}

---

### Bằng chứng trực tiếp & gián tiếp {#direct-indirect-evidence}

---

Trước tiên, chúng ta cần phải hiểu thế nào là “mạng lưới” các phương pháp điều trị. Hãy tưởng tượng rằng chúng tôi đã trích xuất dữ liệu từ một số thử nghiệm ngẫu nhiên có đối chứng $i$, so sánh hiệu quả của phương pháp điều trị A với phương pháp B (ví dụ: nhóm kiểm soát danh sách chờ). Chúng ta có thể minh họa sự so sánh này bằng đồ thị:
...
Hình ảnh biểu diễn so sánh điều trị này được gọi là **đồ thị**. Đồ thị là các cấu trúc được sử dụng để mô hình hóa cách các đối tượng khác nhau liên hệ với nhau, trong đó có hẳn một hướng nghiên cứu trong toán học gọi là **lý thuyết đồ thị**, có liên quan chặt chẽ đến chủ đề này. 

Đồ thị của chúng ta có hai thành phần chính. Thứ đầu tiên là hai vòng tròn (được gọi là **nút**), which đại diện cho hai điều kiện A và B trong thử nghiệm $i$. Thành phần thứ hai là đường nối hai nút này, thường được gọi là **cạnh**. Cạnh biểu thị cách A và B liên quan với nhau. Trong trường hợp này, the việc phiên giải cạnh khá đơn giản. Chúng ta có thể mô tả mối quan hệ giữa A và B bằng mức độ ảnh hưởng $\hat\theta_{i\text{,A,B}}$ chúng ta quan sát được khi so sánh A và B. Mức độ ảnh hưởng này có thể được biểu thị dưới dạng như à SMD, hoặc tỷ số chênh (OR), tùy thuộc vào cách đầu ra được đo lường.
